...
def gen_batch_function(data_folder, image_shape, use_all_training_data=True):
    ...
    :return: The images and their ground truths
    """
    ...
    # Take the first 90% for training and the remaining 10% for evaluation
    img_total = len(image_paths)
    train_total = (int)(len(image_paths)*0.9)
    test_total = img_total - train_total
    print("Images: {0}, TrainingSet: {1}, TestSet: {2}".format(img_total, train_total, test_total ))

    def get_batches_fn(batch_size, dataset_paths):
        """
        Create batches of training data
        :param batch_size: Batch Size
        :return: Batches of training data
        """
        for batch_i in range(0, len(dataset_paths), batch_size):
            images = []
            gt_images = []
            for image_file in dataset_paths[batch_i:batch_i+batch_size]:
